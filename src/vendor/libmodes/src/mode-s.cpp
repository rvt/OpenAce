...
void mode_s_init(mode_s_t *self)
{
  ...
  if (!maglut_initialized)
  {
    for (i = 0; i <= 128; i++)
    {
      for (q = 0; q <= 128; q++)
      {
        maglut[i * 129 + q] = roundf(sqrtf(i * i + q * q) * 360);
      }
    }
...
uint32_t mode_s_checksum(uint8_t *msg, int bits)
{
  uint32_t crc = 0;
...
int brute_force_ap(mode_s_t *self, uint8_t *msg, struct mode_s_msg *mm)
{
...
int32_t decode_ac13_field(const uint8_t *msg, uint8_t *unit)
{
  int m_bit = msg[3] & (1 << 6);
  int q_bit = msg[3] & (1 << 4);
...
int32_t decode_ac12_field(const uint8_t *msg, uint8_t *unit)
{
  int q_bit = msg[5] & 1;
  ...
  else
  {
    ...
    // In the case where the altitude is higher than 50175 feet, a 100 feet increment is used.
    return 50175;
  }
}
...
void mode_s_decode_phase1(mode_s_t *self, struct mode_s_msg *mm, const uint8_t *msg_incomming)
{
  ...
  memcpy(mm->msg, msg_incomming, MODE_S_LONG_MSG_BYTES);
  uint8_t *msg = mm->msg;
    ...
    else if (mm->metype == 19 && mm->mesub >= 1 && mm->mesub <= 4)
    {
      ...
      if (mm->mesub == 1 || mm->mesub == 2)
      {
        ...
        if (mm->velocity)
        {
          ...
          if (mm->ns_dir)
            nsv *= -1;
          heading = atan2f(ewv, nsv);

          // Convert to degrees.
          mm->heading = heading * 360.f / (M_PI * 2.f);
        ...
        if (msg[10] != 0)
        {
          mm->head = ((msg[10] & 0x7F) - 1) * ((msg[10] & 0x80) ? -25 : 25);
        }
        else
        {
          mm->head = 0;
      ...
      else if (mm->mesub == 3 || mm->mesub == 4)
      {
        ...
        mm->heading = (360.f / 128.f) * (((msg[5] & 3) << 5) |
                                         (msg[6] >> 3));
      }
    }
  }
