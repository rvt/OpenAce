...
template< int SIZE_BITS, int PARITY_BITS >
class LDPC_Decoder
{
public:
...
public:
    ...
    {
        int16_t MinAmpl=32767;
        uint8_t MinBit=0;
        int16_t MinAmpl2=MinAmpl;
        uint32_t Word=0;
        uint32_t Mask=1;
...
public:
    ...
    void Input(const uint32_t *Data, Float Ampl=1.0)                      // get bits from a series of 32-bit words
    {
        ...
        for(int Bit=0; Bit<CodeBits; Bit++)
        {
            ...
            OutBit[Bit] = InpBit[Bit] = (Word&Mask) ? +Ampl:-Ampl;
            ExtBit[Bit]=0;
            Mask<<=1;
        }
    }
    ...
    {
        Float MinAmpl=std::numeric_limits<Float>::max();
        int MinBit=0;
        Float MinAmpl2=MinAmpl;               // look for 1st and 2nd smallest LL
        uint32_t Word=0;
        uint32_t Mask=1;
        ...
        for(int Bit=0; Bit<CheckWeight; Bit++)                              // loop over bits in the parity check
        {
            ...
            if(Ampl<MinAmpl)
            {
                MinAmpl2=MinAmpl;    // find 1st and 2nd smallest
                MinAmpl=Ampl;
                MinBit=Bit;
            }
            else if(Ampl<MinAmpl2)
...
public:
    ...
    void Clear(void)
    {
        for(int Symb=0; Symb<CodeSymbols; Symb++)
        {
            ...
            for(int Pulse=0; Pulse<PulsesPerSlot; Pulse++)
            {
                ...
                ExtSymb[Symb][Pulse]=Ext;
                OutSymb[Symb][Pulse]=0;
            }
        }
    }
    ...
    static uint8_t Gray(uint8_t Binary)
    {
