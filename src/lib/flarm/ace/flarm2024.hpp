...
#pragma pack(push, 1)
    struct alignas(uint32_t) RadioPacket
    {
        ...
        int16_t turnRate : 9;                // Bits 128-136: turn rate, degs/sec times 20, enscaled(6,2), signed
        uint16_t groundSpeed : 10;           // Bits 137-146: horizontal speed, m/s times 10, enscaled(8,2)
        int16_t verticalSpeed : 9;           // Bits 147-155: vertical speed, m/s times 10, enscaled(6,2), signed
